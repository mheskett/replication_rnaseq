...
df_repli = pd.read_csv("mouse.rt.mm39.lifted.txt",sep="\t",header=0,index_col=False,
	dtype={"chrom":str,"start":int,"stop":int})
regions=[]
df_repli = df_repli.sort_values(["chrom","start","stop"])
print(df_repli)
for chrom in chromosomes_mouse:
		tmp = df_repli[(df_repli["chrom"]==chrom)].reset_index(drop=True)
		if len(tmp)<=10:
		    continue

		tmp_smoothed = lowess(endog=tmp["mouse_std_dev"],
		                     exog=tmp["start"],
		                      frac=10/len(tmp["start"]),return_sorted=False)
		#### fix these hard coded filters for the peak finding to deal with STD DEV
		#### 
		####
		peaks,properties = find_peaks(tmp_smoothed , width=0, height=0, distance=1, prominence=0.25, threshold=0)
	...
	## but i think youre looking for prominence, which should be in an absolute unit
		...
		plt.margins(0,0)
		# plt.show()
		plt.savefig("mouse."+chrom+".peaks.pdf")
		plt.close()
result = pd.DataFrame(regions,columns=["chrom","start","stop","peak_prominence","peak_width","peak_height","peak_valley"])
...
plt.figure()
plt.hist(result["peak_prominence"],bins=100)
...
plt.figure()
plt.hist(result["peak_height"],bins=100)
# plt.xticks(list(range(0,16)))

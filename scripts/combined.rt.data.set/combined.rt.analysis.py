...
# df_acp6_qn = df_acp6_qn[df_acp6_qn["chrom"]!="chrX"] # dont want to use X in std dev calculation
mean_std_dev = df_acp6_qn["acp6_std_dev_both_haps"].mean()
std_dev_dev = df_acp6_qn["acp6_std_dev_both_haps"].std()
    ...
    for i in range(len(samples)):
        ...
        for j in ["p","q"]:
            ...
            ax[0].plot(paternal[paternal["arm"]==j]["start"],
                    ...
                    c=color_dict_acp6[samples[i]],lw=0.3) ## -- line style is haplotype 2

            ax[0].plot(maternal[maternal["arm"]==j]["start"],
                    maternal[maternal["arm"]==j][samples[i]+"_maternal_logrt"],
                    c=color_dict_acp6[samples[i]],linestyle="--",lw=0.3) ## -- line style is haplotype 2
...
acp7_samples = glob.glob("acp7*counts.bed")
dfs={}
dfs=[]
...
samples=list(set([x[0:7] for x in acp7_samples]))
for sample in samples:
    df_acp7[sample+"_paternal_logrt"] = np.log2((df_acp7[sample+"_p_e_paternal_counts"]+1) / (df_acp7[sample+"_p_l_paternal_counts"]+1 ))
    df_acp7[sample+"_maternal_logrt"] = np.log2((df_acp7[sample+"_m_e_maternal_counts"]+1) / (df_acp7[sample+"_m_l_maternal_counts"]+1 ))
...
# df_acp6_qn = df_acp6_qn[df_acp6_qn["chrom"]!="chrX"] # dont want to use X in std dev calculation
mean_std_dev = df_acp7_qn["acp7_std_dev_both_haps"].mean()
std_dev_dev = df_acp7_qn["acp7_std_dev_both_haps"].std()
threshold = mean_std_dev + 2 *std_dev_dev
...
for chrom in chromosomes:
    ...
    for i in range(len(samples)):
        ...
        for j in ["p","q"]:
            ...
            ax[0].plot(paternal[paternal["arm"]==j]["start"],
                    paternal[paternal["arm"]==j][samples[i]+"_paternal_logrt"],
                    c=color_dict_acp7[samples[i]],lw=0.3) ## -- line style is haplotype 2

            ax[0].plot(maternal[maternal["arm"]==j]["start"],
                    maternal[maternal["arm"]==j][samples[i]+"_maternal_logrt"],
                    c=color_dict_acp7[samples[i]],linestyle="--",lw=0.3) ## -- line style is haplotype 2
...
# df_acp6_qn = df_acp6_qn[df_acp6_qn["chrom"]!="chrX"] # dont want to use X in std dev calculation
mean_std_dev = df_mouse["mouse_std_dev_both_haps"].mean()
std_dev_dev = df_mouse["mouse_std_dev_both_haps"].std()
threshold = mean_std_dev + 2 *std_dev_dev
df_mouse["mouse_vert"] =  df_mouse.apply(lambda x:True if x["mouse_std_dev_both_haps"]>=threshold else False,axis=1)


df_mouse.to_csv("mouse.rt.txt",sep="\t")
df_mouse.to_csv("mouse.rt.bed",sep="\t",header=None,na_rep="NaN")
...
# df_acp6_qn = df_acp6_qn[df_acp6_qn["chrom"]!="chrX"] # dont want to use X in std dev calculation
mean_std_dev = df_eb_qn["eb_std_dev_both_haps"].mean()
std_dev_dev = df_eb_qn["eb_std_dev_both_haps"].std()
threshold = mean_std_dev + 2 *std_dev_dev
...
# df_acp6_qn = df_acp6_qn[df_acp6_qn["chrom"]!="chrX"] # dont want to use X in std dev calculation
mean_std_dev = df_gm_qn["gm_std_dev_both_haps"].mean()
std_dev_dev = df_gm_qn["gm_std_dev_both_haps"].std()
threshold = mean_std_dev + 2 *std_dev_dev
...
df_combined = pd.concat([df_eb_qn,df_gm_qn,df_acp6_qn,df_acp7_qn],axis=1).sort_values(["chrom","start"])
df_combined.to_csv("rt.combined.eb.gm.acp6.acp7.txt",sep="\t")
df_combined.to_csv("rt.combined.eb.gm.acp6.acp7.bed",sep="\t",header=None,na_rep="NaN")

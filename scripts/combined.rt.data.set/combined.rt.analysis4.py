...
acp6_samples = glob.glob("./acp6_113024/acp6*haplotype*counts*rmv.blck*.cpms.*s50.bed")
dfs=[]
...
######
## try gentle smoothing before normalization
print(df_acp6)
df_acp6=df_acp6.reset_index()
df_acp6["arm"] = df_acp6.apply(lambda x: "q" if (x["stop"] > arm_dict_nochr[x["chrom"]][0]) & (x["stop"] <= arm_dict_nochr[x["chrom"]][1]) else "p", axis=1)

for unsmoothed_logrt in df_acp6.columns:
    df_acp6[unsmoothed_logrt+"_smoothed"] = smooth_vector(x=,y=)

print(pd.concat([p,q]))
print(df_acp6["acp6_c5_maternal_logrt"])


exit()
## will adding smoothed columns mess things up here
######
...
for index,row in regions.iterrows():
    ...
    for i in range(len(samples)):
        ...
        ax[0].set_ylim([-4.2,4.2])
        ax[0].set_yticks([-4,-3,-2,-1,0,1,2,3,4])
        ax[0].set_xlim([row["start"]-1500000,row["stop"]+1500000])
        ax[0].set_xticks([])
        ...
        ax[1].set_ylim([0,1.5])
        ax[1].set_yticks([0,1.5])
        ax[1].set_xlim([row["start"]-1500000,row["stop"]+1500000])
        ax[1].set_xticks(np.linspace(row["start"]-1500000,row["stop"]+1500000,5)) 

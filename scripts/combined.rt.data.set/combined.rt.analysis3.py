...
def get_switchers(df):
    ...
    for i in range(len(unique_genes)):
        samples = df_significant_rows[df_significant_rows["gene"]==unique_genes[i]]
...
## updated to mm9 for cedar paper"
chromosome_length_mouse = {"chr1":    197195432,
"chr2":    181748087,
"chr3":    159599783,
"chr4":    155630120,
"chr5":    152537259,
...
df_acp6rna = df_acp6rna[(df_acp6rna["paternal_counts"]+df_acp6rna["maternal_counts"])>=15]
df_acp6rna = df_acp6rna[df_acp6rna["reads_strand"].isin(["plus","minus","antisense"])]
df_acp6rna["80_percent_aei"] = abs(df_acp6rna["aei"]) >= 0.30
df_acp6rna["90_percent_aei"] = abs(df_acp6rna["aei"]) >= 0.40
df_acp6rna["95_percent_aei"] = abs(df_acp6rna["aei"]) >= 0.45
## Stewart et al 2025 nature uses three categories, 80%, 90%, and 95% AEI, with binomial p-value as well
## that would be > abs(.30), > abs(.40), > abs(.45)
add_binom_pval(df_acp6rna)
df_acp6rna = df_acp6rna.reset_index()
df_acp6rna["color"] = df_acp6rna.apply(lambda x: "red" if x["chrom"]=="X" else "blue",axis=1)
# plt.scatter(abs(df_acp6rna["aei"])+0.5, -np.log10(df_acp6rna["fdr_pval"]),
#     s=8,edgecolor="black",lw=0.2) 
# plt.show()

switchers = df_acp6rna[(df_acp6rna["80_percent_aei"]==True) & 
     (df_acp6rna["fdr_pval"]<=0.05) &
     (df_acp6rna["chrom"]!="X")]

sig_genes = df_acp6rna[(df_acp6rna["80_percent_aei"]==True) & 
     (df_acp6rna["fdr_pval"]<=0.05) &
     (df_acp6rna["chrom"]!="X")]

sig_genes.to_csv("acp6_rna.sig.genes.df.txt",index=None,header=None,sep="\t")
os.system("awk '{print $4}' acp6_rna.sig.genes.df.txt | sort | uniq > acp6_rna.all.clones.sig.genes.list.txt")
os.system("awk 'OFS=\"\\t\"{print $1,$2,$3,$4}' acp6_rna.sig.genes.df.txt | sort | uniq > acp6_rna.all.clones.sig.genes.list.bed")
# np.savetxt(fname="acp6_rna_aei_genes.txt", X=pd.unique(sig_genes["gene"]),delimiter="\t")

# sig_genes.to_csv("acp6_rna_aei_genes.txt",columns = ["gene"],sep="\t",index=None,header=None)
switchers = get_switchers(df_acp6rna)
####
####
####
####
...
acp6_samples = glob.glob("./acp6_113024/acp6*haplotype*counts*rmv.blck*.cpms.*s250.bed")
# acp6_samples = glob.glob("./acp6_113024/acp6*haplotype*counts*rmv.blck*.cpms.*s50.smoothed.bed")

dfs=[]
## now we have to remove "." data points
...
###### filter based on CPMS
print(df_acp6)
# for sample in samples:
#     sns.kdeplot(df_acp6[sample+"_early_cpm_p_counts"],clip=(0,250),c="blue")
#     sns.kdeplot(df_acp6[sample+"_early_cpm_m_counts"],clip=(0,250),c="blue")
#     sns.kdeplot(df_acp6[sample+"_late_cpm_p_counts"],clip=(0,250),c="red")
#     sns.kdeplot(df_acp6[sample+"_late_cpm_m_counts"],clip=(0,250),c="red")
#     plt.show()
# df_acp6=df_acp6[df_acp6[""]]
...
print("before dropping", len(df_acp6))
print("after dropping", len(df_acp6.dropna(how="any",axis=0)))
# df_acp6=df_acp6.dropna(how="any",axis=0) # try not filtering
    ...
    for i in range(len(samples)):
        ...
        for j in ["p","q"]:
            ...
            ax[0].plot(paternal[paternal["arm"]==j]["start"],
                    ...
                    c=color_dict_acp6[samples[i]],lw=0.75) ## -- line style is haplotype 2

            ax[0].plot(maternal[maternal["arm"]==j]["start"],
                    maternal[maternal["arm"]==j][samples[i]+"_maternal_logrt"],
                    c=color_dict_acp6[samples[i]],linestyle="--",lw=0.75) ## -- line style is haplotype 2
...
os.system("sort -k1,1 -k2,2n acp6.rt.bed | grep True | awk '$1!=\"X\"{print $0}' > acp6.rt.vert.sorted.bed")
os.system("bedtools map -a acp6.rt.vert.sorted.bed -b ucsc.known.gene.hg19.txn.start.stop.bed.cds.only.first.isoform.nochr.sorted.bed -o distinct -c 4 > acp6.rt.vert.intersect.coding.bed ")
os.system("awk '{print $15}'  acp6.rt.vert.intersect.coding.bed | awk '{$1=$1} 1' FS=, OFS='\\n'| sort | uniq > acp6.vert.genes.txt")
...
####
####
acp7_samples = glob.glob("./acp7_120324/acp7*haplotype*counts*rmv.blck*.cpms.*s50.bed")
...
f,ax=plt.subplots(figsize=(2,1),dpi=300)
...
plt.savefig("acp7.vert.dist.png",bbox_inches='tight')
...
plt.savefig("eb.vert.norm.dist.png")
...
os.system("sort -k1,1 -k2,2n acp7.rt.bed | grep True | awk '$1!=\"X\"{print $0}' > acp7.rt.vert.sorted.bed")
os.system("bedtools map -a acp7.rt.vert.sorted.bed -b ucsc.known.gene.hg19.txn.start.stop.bed.cds.only.first.isoform.nochr.sorted.bed -o distinct -c 4 > acp7.rt.vert.intersect.coding.bed ")
os.system("awk '{print $13}'  acp7.rt.vert.intersect.coding.bed | awk '{$1=$1} 1' FS=, OFS='\\n'| sort | uniq > acp7.vert.genes.txt")
...
for index,row in regions.iterrows():
    ...
    for i in range(len(samples)):
        ...
        paternal = paternal[(paternal["chrom"]==row["chrom"]) & (paternal["start"]>=row["start"]-1500000) & (paternal["stop"]<=row["stop"]+1500000)]
        maternal = maternal[(maternal["chrom"]==row["chrom"]) & (maternal["start"]>=row["start"]-1500000) & (maternal["stop"]<=row["stop"]+1500000)]
        ...
        rna_tmp = df_acp7rna[(df_acp7rna["chrom"]==row["chrom"]) & 
                            (df_acp7rna["start"]>=row["start"]-300000) & 
                            (df_acp7rna["stop"]<=row["stop"]+300000) & 
        ...
        ax[0].set_ylim([-4.2,4.2])
        ax[0].set_yticks([-4,-3,-2,-1,0,1,2,3,4])
        ax[0].set_xlim([row["start"]-1200000,row["stop"]+1200000])
        ax[0].set_xticks([])
        ...
        ax[1].set_ylim([0,1.5])
        ax[1].set_yticks([0,1.5])
        ax[1].set_xlim([row["start"]-1200000,row["stop"]+1200000])
        ax[1].set_xticks(np.linspace(row["start"]-1200000,row["stop"]+1200000,5)) 
...
## FIX PLOTTING AND GENE ANNOTATIONS
## NEEDS TO BE mm9 NOT mm10!!!!
...
plt.savefig("mouse.vert.dist.png")
...
a = pybedtools.BedTool.from_dataframe(regions)
regions = a.merge(d=100001).filter(lambda x: len(x) > 100000).saveas().to_dataframe().reset_index()
regions = regions.drop("index",axis="columns")
regions.columns = ["chrom","start","stop"]
regions["chrom"] = regions["chrom"].astype(str)
print(regions)
...
os.system("sort -k1,1 -k2,2n mouse.rt.bed | grep True | awk '$1!=\"chrX\"{print $0}' > mouse.rt.vert.sorted.bed")
...
os.system("awk '{print $24}'  mouse.rt.vert.intersect.coding.bed  | awk '{$1=$1} 1' FS=, OFS='\\n'| sort | uniq > mouse.vert.genes.txt")



#### need to create the mouse gene file for VERT 
...
### RNA
dfs=[]
eb_rna = glob.glob("../rna.dec2024/eb*haplotype.resolved.counts.comprehensive.gene.counts.bed")
for f in eb_rna:
    samp=os.path.basename(f).split(".")[0].split("Aligned")[0]
    tmp = pd.read_csv(f,sep="\t",names=["chrom","start","stop","gene","score","gene_strand",
                             "tx_start","tx_stop","paternal_counts","maternal_counts","reads_strand"])
    ...
    tmp["aei"] =  tmp.apply(helper_func, axis = 1)
    dfs += [tmp]
df_ebrna = pd.concat(dfs).sort_values(["chrom","start"])
add_binom_pval(df_ebrna)
df_ebrna = df_ebrna.reset_index()

print(df_ebrna)
### 
eb_samples = glob.glob("eb*rt*counts*rmv*blck*cpm*windows.s50.bed")
dfs=[]
for x in eb_samples:
    x_samp="_".join(x.split("_")[0:4])
    tmp=pd.read_csv(x,sep="\t",names=["chrom" ,"start", "stop", x_samp+"_cpm_hap1_counts", x_samp+"_cpm_hap2_counts"])
    # tmp=tmp[tmp["chrom"]!="X"]###removing chrom x 
    tmp=tmp[tmp["chrom"]!="Y"]###removing chrom x 
    tmp = tmp[(tmp[x_samp+"_cpm_hap1_counts"]!=".") & (tmp[x_samp+"_cpm_hap2_counts"]!=".")]
    tmp[x_samp+"_cpm_hap1_counts"] = tmp[x_samp+"_cpm_hap1_counts"].astype(float)
    tmp[x_samp+"_cpm_hap2_counts"] = tmp[x_samp+"_cpm_hap2_counts"].astype(float)
    tmp=tmp.set_index(["chrom","start","stop"])
...
color_dict_eb_rna = {'eb3_2_clone15':"red", 
'eb3_2_clone4':"cyan", 
'eb3_2_clone13':"yellow", 
'eb3_2_clone10':"green", 
'eb3_2_clone2':"blue", 
...
os.system("sort -k1,1 -k2,2n eb32.rt.vert.bed | grep True | awk '$1!=\"X\"{print $0}' > eb32.rt.vert.sorted.bed")
...
os.system("awk '{print $19}'  eb32.rt.vert.intersect.coding.bed | awk '{$1=$1} 1' FS=, OFS='\\n'| sort | uniq > eb32.vert.genes.txt")
...
for index,row in regions.iterrows():
    ...
    for i in range(len(eb_samples)):
        ...
        #######
        ax2 = ax[0].twinx()
        rna_tmp = df_ebrna[(df_ebrna["chrom"]==row["chrom"]) & 
                            (df_ebrna["start"]>=row["start"]-300000) & 
                            (df_ebrna["stop"]<=row["stop"]+300000) & 
                            (df_ebrna["total_reads"]>=10) & 
                            (df_ebrna["reads_strand"].isin(["plus","minus"]))]
        ...
        ax[0].set_ylim([-3.2,3.2])
        ax[0].set_yticks([-3,-2,-1,0,1,2,3])
        ax[0].set_xlim([row["start"]-1200000,row["stop"]+120000])
        ax[0].set_xticks([])
        ...
        ax[1].set_yticks([0,1.5])
        ax[1].set_xlim([row["start"]-1200000,row["stop"]+1200000])
        ax[1].set_xticks(np.linspace(row["start"]-1200000,row["stop"]+120000,5)) 
...
################################
dfs=[]
gm_rna = glob.glob("../rna.dec2024/gm*haplotype.resolved.counts.comprehensive.gene.counts.bed")
for f in gm_rna:
    samp=os.path.basename(f).split(".")[0].split("Aligned")[0]
    tmp = pd.read_csv(f,sep="\t",names=["chrom","start","stop","gene","score","gene_strand",
                             "tx_start","tx_stop","paternal_counts","maternal_counts","reads_strand"])
    ...
    tmp["aei"] =  tmp.apply(helper_func, axis = 1)
    dfs += [tmp]
df_gmrna = pd.concat(dfs).sort_values(["chrom","start"])
add_binom_pval(df_gmrna)
df_gmrna = df_gmrna.reset_index()



gm_samples = glob.glob("gm*rt*counts*rmv.blck*cpm*windows.s50.bed")
dfs=[]
for x in gm_samples:
...
os.system("sort -k1,1 -k2,2n gm.rt.vert.bed | grep True | awk '$1!=\"X\"{print $0}' > gm.rt.vert.sorted.bed")
...
os.system("awk '{print $11}' gm.rt.vert.intersect.coding.bed | awk '{$1=$1} 1' FS=, OFS='\\n'| sort | uniq > gm.vert.genes.txt")



color_dict_gm = {'gm12878_clone5':"red", 'gm12878_clone4' :"blue"}
color_dict_gm_rna = {'gm12878_clone5':"red", 'gm12878_clone4' :"blue"}
...
for index,row in regions.iterrows():
    ...
    for i in range(len(gm_samples)):
        ...
        #######
        ax2 = ax[0].twinx()
        rna_tmp = df_gmrna[(df_gmrna["chrom"]==row["chrom"]) & 
                            (df_gmrna["start"]>=row["start"]-300000) & 
                            (df_gmrna["stop"]<=row["stop"]+300000) & 
                            (df_gmrna["total_reads"]>=10) & 
                            (df_gmrna["reads_strand"].isin(["plus","minus"]))]

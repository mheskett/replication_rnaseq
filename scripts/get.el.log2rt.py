...
early_df= pd.read_csv(argv[1],sep="\t",
                        names= ["chrom","start","stop","coverage_early"],
                        dtype = {"chrom":str,"start":int,"stop":int,"log2rt":float}).set_index(["chrom","start","stop"])
#early_df["sample"] = os.path.basename(argv[1])[0:11]

late_df= pd.read_csv(argv[2],sep="\t",
                        names= ["chrom","start","stop","coverage_late"],
                        dtype = {"chrom":str,"start":int,"stop":int,"log2rt":float}).set_index(["chrom","start","stop"])
#late_df["sample"] = os.path.basename(argv[2])[0:11]

# print(early_df)
# print(late_df)
df=pd.concat([early_df,late_df],axis=1).reset_index()

df["early_lsm"] = df["coverage_early"] / df["coverage_early"].sum(axis="rows") * 10**6
df["late_lsm"] = df["coverage_late"] / df["coverage_late"].sum(axis="rows") * 10**6
df["log2rt"] = np.log2((df["early_lsm"]+1)/(df["late_lsm"]+1))
#df=df[~((df["early_lsm"]==0) & (df["late_lsm"]==0))]
df=df[df["log2rt"]!=0]
print(df)

df[["chrom","start","stop","log2rt"]].to_csv(argv[1][0:10]+"log2rt.bed",sep="\t",header=False,index=False)

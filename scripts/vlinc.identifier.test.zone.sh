...
bedtools merge -i $out_dir$filename.plus.subtract.merge.1kb.bed -d 10000 > $out_dir$filename.plus.subtract.merge.1kb.filter.bed
bedtools merge -i $out_dir$filename.minus.subtract.merge.1kb.bed -d 10000 > $out_dir$filename.minus.subtract.merge.1kb.filter.bed

## now only keep fragments of size 50kb or larger. could consider lowering or removing this.
## this is the most stringent part since there are small gaps that will prevent 50kb fragments from existing
awk '$3-$2>=50000{print $0}' $out_dir$filename.plus.subtract.merge.1kb.filter.bed >  $out_dir$filename.plus.vlinc.discovery.bed
awk '$3-$2>=50000{print $0}' $out_dir$filename.minus.subtract.merge.1kb.filter.bed >  $out_dir$filename.minus.vlinc.discovery.bed
...
bash hg38.to.hg19.sh $out_dir$filename.vlinc.discovery.plus.browser.bed $out_dir$filename.vlinc.discovery.plus.browser.hg19.bed
bash hg38.to.hg19.sh $out_dir$filename.vlinc.discovery.minus.browser.bed $out_dir$filename.vlinc.discovery.minus.browser.hg19.bed

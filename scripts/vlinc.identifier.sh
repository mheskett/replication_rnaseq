...
bedtools subtract -a  $out_dir$filename.mq40.plus.cov.bed -b /home/groups/Spellmandata/heskett/replication.rnaseq/annotation.files/whole.gene.plus.nochr.ucsc.hg38.bed > $out_dir$filename.plus.subtract.whole.gene.bed
bedtools subtract -a  $out_dir$filename.mq40.minus.cov.bed -b /home/groups/Spellmandata/heskett/replication.rnaseq/annotation.files/whole.gene.minus.nochr.ucsc.hg38.bed > $out_dir$filename.minus.subtract.whole.gene.bed
...
awk 'OFS="\t"{print "chr"$1,$2,$3,"vlinc",NR,"+"}' $out_dir$filename.plus.vlinc.discovery.bed >> $out_dir$filename.vlinc.discovery.browser.bed
awk 'OFS="\t"{print "chr"$1,$2,$3,"vlinc",NR,"-"}' $out_dir$filename.minus.vlinc.discovery.bed >> $out_dir$filename.vlinc.discovery.browser.bed

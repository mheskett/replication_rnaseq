...
repli_df["low_coverage"] = [True if x <=250 else False for x in repli_df["total_reads"]]
...
tmp_coding[tmp_coding["significant_deviation"]==True]["name"].drop_duplicates().to_csv("coding.genes.in.vert.regions.txt",sep="\t",index=False,header=False)
...
# for index,row in thayer_fish_loci.drop_duplicates(["chrom","start","stop"]).iterrows():
    ...
    stop=row["stop"]
    chrom=str(row["chrom"])
    plt.suptitle(chrom+","+str(row["zscore_abs_diff_hap_means"]))
    ax_lnc = ax.twinx()
    # df is lncs

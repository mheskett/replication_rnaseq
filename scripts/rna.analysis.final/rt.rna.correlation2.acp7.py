...
## TLs
df_eb_tls = pd.read_csv("acp7.as.tl.counts.all.txt",sep="\t")

## repli
df_eb_rt = pd.read_csv("../combined.rt.data.set/acp7.rt.txt",sep='\t',dtype={"acp7_std_dev_both_haps":float})
vert_rt_windows = df_eb_rt[(df_eb_rt["acp7_vert"]==True) & (df_eb_rt["chrom"]!="chrX")].loc[:,["chrom","start","stop"]].drop_duplicates()
...
all_rt_windows_bed_unmerged = pybedtools.BedTool.from_dataframe(df_eb_rt[(df_eb_rt["chrom"]!="chrX") ].loc[:,["chrom","start","stop","acp7_std_dev_both_haps"]]).sort()
...
### regular genes
df_eb_genes = pd.read_csv("acp7.as.gene.counts.txt",sep="\t")
...
# print(df_eb_genes[(df_eb_genes["fdr_pval"]<=0.05) & (abs(df_eb_genes["aei"])>=0.2)])


df_eb_genes['aei_std_dev'] = df_eb_genes.groupby('name')['aei'].transform('std')
print(df_eb_genes)
df_eb_genes["variable_aei"] = df_eb_genes.groupby("name")["aei_std_dev"].transform("mean") >= np.nanpercentile(df_eb_genes.groupby("name")["aei_std_dev"].transform("mean"), 97.5)
df_eb_genes["aei_70%_sig"] = (df_eb_genes["fdr_pval"]<=0.05) & (abs(df_eb_genes["aei"])>=0.2)
# print(df_eb_genes[((df_eb_genes["fdr_pval"]<=0.05) & (abs(df_eb_genes["aei"])>=0.2)) | (df_eb_genes["variable_aei"]==True)])
#### fdr < 0.5 and aei >=0.2 OR variable aei == True

df_eb_genes["aei_>=_one_clone"] = df_eb_genes.groupby('name')['aei_70%_sig'].transform('any')

print(df_eb_genes[(df_eb_genes["aei_>=_one_clone"]==True) | (df_eb_genes["variable_aei"]==True)].drop_duplicates("name"))
# print(df_eb_genes[(df_eb_genes["variable_aei"]==True)].drop_duplicates("name"))
# print(df_eb_genes["variable_aei"])

# tmp_aei_df["variable_aei"] = tmp_aei_df['aei_std_dev'] >= np.percentile(tmp_aei_df['aei_std_dev'], 97.5)
# print(tmp_aei_df[tmp_aei_df["variable_aei"]==True]["name"])
# variable_aei_gene_list = tmp_aei_df[tmp_aei_df["variable_aei"]==True].sort_values(["aei_std_dev"],ascending=False).drop_duplicates("name")["name"]
# print(list(variable_aei_gene_list))
exit()
...
gene_aei_intersect_all_rt_windows_df.columns = ["chrom","start","stop","name","aei_std_dev",
                                        "chrom_rt","start_rt","stop_rt","acp7_std_dev_both_haps"]


### try a fisher exact test
gene_aei_intersect_all_rt_windows_df["variable_aei"] = gene_aei_intersect_all_rt_windows_df['aei_std_dev'] >= np.percentile(gene_aei_intersect_all_rt_windows_df['aei_std_dev'], 95)
gene_aei_intersect_all_rt_windows_df["variable_rt"] = gene_aei_intersect_all_rt_windows_df['acp7_std_dev_both_haps'] >= np.percentile(gene_aei_intersect_all_rt_windows_df['acp7_std_dev_both_haps'], 97.5)

gene_aei_intersect_all_rt_windows_df = gene_aei_intersect_all_rt_windows_df.sort_values(["aei_std_dev"],ascending=False).drop_duplicates("name")

print(gene_aei_intersect_all_rt_windows_df)

a = np.sum((gene_aei_intersect_all_rt_windows_df["variable_aei"]==True) & (gene_aei_intersect_all_rt_windows_df["variable_rt"]==True))     # both high
print("num aei high rt high",a)
b = np.sum((gene_aei_intersect_all_rt_windows_df["variable_aei"]==True) & (gene_aei_intersect_all_rt_windows_df["variable_rt"]==False))    # X high, Y low
print("num aei high rt low",b)
c = np.sum((gene_aei_intersect_all_rt_windows_df["variable_aei"]==False) & (gene_aei_intersect_all_rt_windows_df["variable_rt"]==True))    # X low, Y high
print("num aei low rt high",c)
d = np.sum((gene_aei_intersect_all_rt_windows_df["variable_aei"]==False) & (gene_aei_intersect_all_rt_windows_df["variable_rt"]==False))   # both low
print("num aei low rt low",d)
## 97th percentile seems highest
oddsratio, pval = fisher_exact([[a, b], [c, d]], alternative='greater')

print(gene_aei_intersect_all_rt_windows_df[gene_aei_intersect_all_rt_windows_df["variable_aei"]==True])
...
# gene_aei_intersect_all_rt_windows_df["variable_rt"] = gene_aei_intersect_all_rt_windows_df['acp7_std_dev_both_haps'] >= np.percentile(gene_aei_intersect_all_rt_windows_df['acp7_std_dev_both_haps'], 97)
...
print(high_aei_high_rt.sort_values(["aei_std_dev"],ascending=False))
exit()
# print(gene_aei_intersect_all_rt_windows_df) 

# pearson
corr, p_value = pearsonr(gene_aei_intersect_all_rt_windows_df["aei_std_dev"].astype(float), 
    gene_aei_intersect_all_rt_windows_df["acp7_std_dev_both_haps"].astype(float))
print("pearson Correlation:", corr)
print("P-value:", p_value)

# spearman
# corr, p_value = spearmanr(gene_aei_intersect_rt_vert_df["aei_std_dev"].astype(float), 
#     gene_aei_intersect_rt_vert_df["acp6_std_dev_both_haps"].astype(float))
...
#     np.log(gene_aei_intersect_rt_vert_df["acp6_std_dev_both_haps"].astype(float)))
...
all_genes_bed.to_dataframe(disable_auto_names=True, header=None).to_csv("all_genes_acp7.bed",sep="\t",header=None,index=None)
...
vert_rt_windows_bed.intersect(sig_genes_bed,wa=True,wb=True).to_dataframe(disable_auto_names=True, header=None).to_csv("acp7.vert.rt.windows.bed",sep="\t",index=None)
sim_results=[]
...
for i in range(10000):
    ...
    shuffle_genes = sig_genes_bed.shuffle(g="../hg38.fa.fai", incl="all_genes_acp7.bed",chromFirst=True)
    sim_results += [len(shuffle_vert.intersect(shuffle_genes,wa=True,wb=True))]

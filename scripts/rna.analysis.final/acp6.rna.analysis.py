...
#####
#RNA
acp6_samples = glob.glob("./acp6_c*_rna*rmv.blck.comprehensive.gene.counts.bed")
...
pd.Series(switchers["name"].unique()).to_csv("acp6.rna.switchers.txt",sep="\t",index=False,header=False)

print(switchers)
df_acp6.to_csv("acp6.as.gene.counts.txt",sep="\t",index=None)
df_acp6_with_x.to_csv("acp6.with.x.as.gene.counts.txt",sep="\t",index=None)
##
color_dict_acp6 = {
...
tmp = df_acp6[df_acp6["name_strand"].isin(switchers["name_strand"])]
tmp["color"]= [color_dict_acp6[x] for x in tmp["sample"]]
tmp["alpha"] = tmp.apply(lambda row: 1 if (row["fdr_reject"]==True) and (abs(row["aei"])>=0.2) else 0.1, axis=1)
# df['new_col'] = df.apply(lambda row: 'condition_met' if (row['col1'] > 2) and (row['col2'] < 8) else 'condition_not_met', axis=1)

tmp["alpha"] = [1 if x==True else 0.1 for x in tmp["fdr_reject"]]
...
plt.savefig("acp6.rna.switchers.png",

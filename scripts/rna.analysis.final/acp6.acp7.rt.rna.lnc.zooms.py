...
## repli
df_acp7_rt = pd.read_csv("../combined.rt.data.set/acp7.rt.txt",sep='\t')
regions = df_acp7_rt[(df_acp7_rt["acp7_vert"]==True) & (df_acp7_rt["chrom"]!="chrX")]
...
rt_samples_acp7= ["acp7_c2","acp7_c4","acp7_c5"]
rna_samples_acp7 = ["acp7_c1_rnaAligned","acp7_c2_rnaAligned","acp7_c4_rnaAligned"]
...
rt_samples_acp6= ["acp6_c1","acp6_c2","acp6_c5","acp6_c6"]
rna_samples_acp6 = ["acp6_c1_rnaAligned","acp6_c2_rnaAligned","acp6_c5_rnaAligned","acp6_c6_rnaAligned"]
...
for index,row in regions.iterrows():
    ...
    ## RT
    for i in range(len(rt_samples_acp6)):

        paternal = df_acp6_rt[(df_acp6_rt["chrom"]==row["chrom"])].set_index(["chrom","start","stop","arm"]).filter(like=rt_samples_acp6[i]+"_paternal_logrt").reset_index()
        maternal = df_acp6_rt[(df_acp6_rt["chrom"]==row["chrom"])].set_index(["chrom","start","stop","arm"]).filter(like=rt_samples_acp6[i]+"_maternal_logrt").reset_index()
        ...
        for j in ["p","q"]:
            ...
            ax.plot(paternal[paternal["arm"]==j]["start"],
                    paternal[paternal["arm"]==j][rt_samples_acp6[i]+"_paternal_logrt"],
                    c="blue",lw=1) ## -- line style is haplotype 2

            ax.plot(maternal[maternal["arm"]==j]["start"],
                    maternal[maternal["arm"]==j][rt_samples_acp6[i]+"_maternal_logrt"],
                    c="blue",linestyle="--",lw=1) ## -- line style is haplotype 2
        ax.set_ylim([-4,4])
    ...
    for i in range(len(rt_samples_acp7)):

        paternal = df_acp7_rt[(df_acp7_rt["chrom"]==row["chrom"])].set_index(["chrom","start","stop","arm"]).filter(like=rt_samples_acp7[i]+"_paternal_logrt").reset_index()
        maternal = df_acp7_rt[(df_acp7_rt["chrom"]==row["chrom"])].set_index(["chrom","start","stop","arm"]).filter(like=rt_samples_acp7[i]+"_maternal_logrt").reset_index()
        ...
        for j in ["p","q"]:
            ## unsmoothed
            ax.plot(paternal[paternal["arm"]==j]["start"],
                    paternal[paternal["arm"]==j][rt_samples_acp7[i]+"_paternal_logrt"],
                    c="lime",lw=1) ## -- line style is haplotype 2

            ax.plot(maternal[maternal["arm"]==j]["start"],
                    maternal[maternal["arm"]==j][rt_samples_acp7[i]+"_maternal_logrt"],
                    c="lime",linestyle="--",lw=1) ## -- line style is haplotype 2
...
#         ax2.set_yticks([-0.5,-0.4,-0.3,-0.2,-0.1, 0, .1, .2, .3, .4, 0.5])
    ...
    plt.suptitle("acp6.acp7."+str(row["chrom"])+":"+str(row["start"]))
    # plt.savefig("acp6.acp7.rt.genes.tls."+str(row["chrom"])+"-"+str(row["start"])+".png",
...
### RNA coding genes
df_acp7_genes = pd.read_csv("acp7.as.gene.counts.txt",sep="\t")

## TLs
# df_acp7_tls = pd.read_csv("acp7.as.tl.counts.all.txt",sep="\t")
...
for index,row in regions.iterrows():
    ...
    for i in range(len(rt_samples)):
        ...
        paternal = df_acp7_rt[(df_acp7_rt["chrom"]==row["chrom"])].set_index(["chrom","start","stop","arm"]).filter(like=rt_samples[i]+"_paternal_logrt").reset_index()
        maternal = df_acp7_rt[(df_acp7_rt["chrom"]==row["chrom"])].set_index(["chrom","start","stop","arm"]).filter(like=rt_samples[i]+"_maternal_logrt").reset_index()
        ...
        for j in ["p","q"]:
            ...
            ax.plot(paternal[paternal["arm"]==j]["start"],
                    ...
                    c=color_dict_acp7[rt_samples[i]],lw=0.75) ## -- line style is haplotype 2

            ax.plot(maternal[maternal["arm"]==j]["start"],
                    maternal[maternal["arm"]==j][rt_samples[i]+"_maternal_logrt"],
                    c=color_dict_acp7[rt_samples[i]],linestyle="--",lw=0.75) ## -- line style is haplotype 2
        ax.set_ylim([-4,4])
        ax.set_yticks([-4,-3,-2,-1,0,1,2,3,4])
        ax.set_xlim([row["start"]-1000000,row["stop"]+1000000]) # needs to be matched with the below xlim
        ax.set_xticks(np.linspace(row["start"]-1000000,row["stop"]+1000000,5)) 
...
print(df_acp7_genes, df_acp7_tls, df_acp7_rt)

...
rt_samples= ["acp7_c2","acp7_c4","acp7_c5"]
rna_samples = ["acp7_c1_rnaAligned","acp7_c2_rnaAligned","acp7_c4_rnaAligned"]
...
### coding genes
        ...
        ax2 = ax.twinx()
        rna_tmp = df_acp7_genes[(df_acp7_genes["chrom"]==row["chrom"]) & 
                            (df_acp7_genes["start"]>=row["start"]-300000) & 
                            (df_acp7_genes["stop"]<=row["stop"]+300000) & 
                            (df_acp7_genes["total_reads"]>=10) & 
                            (df_acp7_genes["strand_reads"].isin(["plus","minus"]))]
        ...
        for index2, row2 in rna_tmp.iterrows():
            ...
            ax2.add_patch(rect)
### TLS
        tl_tmp = df_acp7_tls[(df_acp7_tls["chrom"]==row["chrom"]) & 
                            (df_acp7_tls["start"]>=row["start"]-300000) & 
                            (df_acp7_tls["stop"]<=row["stop"]+300000) & 
                            (df_acp7_tls["total_reads"]>=10) &
                            (df_acp7_tls["strand_reads"].isin(["plus","minus"]))]

        for index2, row2 in tl_tmp.iterrows():
            rect=Rectangle((row2["start"], row2["aei"]-.0125), 
                            width=row2["stop"]-row2["start"], height=0.025,
                            facecolor=color_dict_acp7_tls[row2["sample"]], edgecolor="black",
                            fill=True,lw=.5)
    ...
    plt.savefig("acp7.rt.genes.tls."+str(row["chrom"])+"-"+str(row["start"])+".png",

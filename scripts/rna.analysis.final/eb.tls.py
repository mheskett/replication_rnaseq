...
eb_samples = glob.glob("./eb*as.tl.counts.bed")
...
# sns.kdeplot(df_eb["informative_reads_per_kb"],clip=(0,20))
# plt.show()
# plt.close()
# sns.kdeplot(df_eb["total_reads"],clip=(0,1000))
# plt.show()

# switchers=get_switchers(df_eb)
# pd.Series(switchers["name"].unique()).to_csv("eb.tls.switchers.txt",sep="\t",index=False,header=False)

df_eb.to_csv("eb.as.tl.counts.all.txt",sep="\t",index=None)

...
#####
#RNA
acp7_samples = glob.glob("./acp7_c*_rna*rmv.blck.comprehensive.gene.counts.bed")
...
pd.Series(switchers["name"].unique()).to_csv("acp7.rna.switchers.txt",sep="\t",index=False,header=False)

print(switchers)
df_acp7.to_csv("acp7.as.gene.counts.txt",sep="\t",index=None)
df_acp7_with_x.to_csv("acp7.with.x.as.gene.counts.txt",sep="\t",index=None)
...
tmp = df_acp7[df_acp7["name_strand"].isin(switchers["name_strand"])]

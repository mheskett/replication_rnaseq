...
## TLs
df_eb_tls = pd.read_csv("acp6.as.tl.counts.all.txt",sep="\t")

## repli
df_eb_rt = pd.read_csv("../combined.rt.data.set/acp6.rt.txt",sep='\t',dtype={"acp6_std_dev_both_haps":float})
vert_rt_windows = df_eb_rt[(df_eb_rt["acp6_vert"]==True) & (df_eb_rt["chrom"]!="chrX")].loc[:,["chrom","start","stop"]].drop_duplicates()
...
all_rt_windows_bed_unmerged = pybedtools.BedTool.from_dataframe(df_eb_rt[(df_eb_rt["chrom"]!="chrX") ].loc[:,["chrom","start","stop","acp6_std_dev_both_haps"]]).sort()
...
### regular genes
df_eb_genes = pd.read_csv("acp6.as.gene.counts.txt",sep="\t")
...
gene_aei_intersect_all_rt_windows_df.columns = ["chrom","start","stop","name","aei_std_dev",
                                        "chrom_rt","start_rt","stop_rt","acp6_std_dev_both_haps"]
...
rt_high = gene_aei_intersect_all_rt_windows_df['acp6_std_dev_both_haps'] >= np.percentile(gene_aei_intersect_all_rt_windows_df['acp6_std_dev_both_haps'], 97)
...
gene_aei_intersect_all_rt_windows_df["variable_rt"] = gene_aei_intersect_all_rt_windows_df['acp6_std_dev_both_haps'] >= np.percentile(gene_aei_intersect_all_rt_windows_df['acp6_std_dev_both_haps'], 97)
...
corr, p_value = pearsonr(gene_aei_intersect_all_rt_windows_df["aei_std_dev"].astype(float), 
    gene_aei_intersect_all_rt_windows_df["acp6_std_dev_both_haps"].astype(float))
...
all_genes_bed.to_dataframe(disable_auto_names=True, header=None).to_csv("all_genes_acp6.bed",sep="\t",header=None,index=None)
...
vert_rt_windows_bed.intersect(sig_genes_bed,wa=True,wb=True).to_dataframe(disable_auto_names=True, header=None).to_csv("acp6.vert.rt.windows.bed",sep="\t",index=None)
sim_results=[]
...
for i in range(10000):
    ...
    shuffle_genes = sig_genes_bed.shuffle(g="../hg38.fa.fai", incl="all_genes_acp6.bed",chromFirst=True)
    sim_results += [len(shuffle_vert.intersect(shuffle_genes,wa=True,wb=True))]

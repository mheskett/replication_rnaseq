...
## TLs
# df_eb_tls = pd.read_csv("gm.as.tl.counts.all.hg.txt",sep="\t")

## repli
df_eb_rt = pd.read_csv("../combined.rt.data.set/gm.rt.txt",sep='\t')
...
all_rt_windows_bed.to_dataframe(disable_auto_names=True, header=None).to_csv("all_rt_windows_gm.bed",sep="\t",header=None,index=None)
...
all_genes_bed = pybedtools.BedTool.from_dataframe(df_gm_genes[df_gm_genes["chrom"]!="chrX"]).sort().merge()
sig_genes_bed = pybedtools.BedTool.from_dataframe(df_gm_genes[(df_gm_genes["fdr_reject"]==True) & 
                                    (df_gm_genes["chrom"]!="chrX") &
                                    (df_gm_genes["70_percent_aei"]==True) &
                                    (df_gm_genes["fdr_pval"]<=0.001)].drop_duplicates(["chrom","start","stop"])).sort()#.merge()
...
###
vert_intersect_genes = vert_rt_windows_bed.intersect(sig_genes_bed,wa=True,wb=True)
real_result = len(vert_intersect_genes)
...
print("real intersections", real_result)
print(vert_intersect_genes)


# shuffle vert and genes
# for i in range(5000):
#     shuffle_vert = vert_rt_windows_bed.shuffle(g="../hg38.fa.fai", incl="all_rt_windows_gm.bed",chromFirst=False,noOverlapping=True)
#     shuffle_genes = sig_genes_bed.shuffle(g="../hg38.fa.fai", incl="all_genes_gm.bed",chromFirst=False,noOverlapping=True)
...
## shuffle vert only
for i in range(5000):
    shuffle_vert = vert_rt_windows_bed.shuffle(g="../hg38.fa.fai", incl="all_rt_windows_gm.bed",chromFirst=True)
    shuffle_genes = sig_genes_bed#.shuffle(g="../hg38.fa.fai", incl="all_genes.bed",chromFirst=True)
    sim_results += [len(shuffle_vert.intersect(shuffle_genes,wa=True,wb=True))]

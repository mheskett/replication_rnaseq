...
repli_df = repli_df[repli_df["chrom"]!="chrY"]
...
df_normalized_paternal["std_dev"] = df_normalized_paternal.filter(like="acp7",axis=1).std(axis="columns")
df_normalized_maternal["std_dev"] = df_normalized_maternal.filter(like="acp7",axis=1).std(axis="columns")
...
df_normalized_both_haps["std_dev"] = df_normalized_both_haps.filter(like="acp7",axis=1).std(axis="columns")
...
df_normalized_both_haps.to_csv("acp7.rt.vert.txt",sep="\t",header=True,index=False)
df_normalized_both_haps.to_csv("acp7.rt.vert.bed",sep="\t",header=False,index=False)

## change appropriately. color is clonal sample, then dotted vs colored is maternal paternal
# print("df_normalized both haps: ",df_normalized_both_haps)
color_dict_repli = {"acp7_c2_m_el":"cyan",  
"acp7_c2_p_el":"cyan",
"acp7_c5_m_el":"red",  
"acp7_c5_p_el":"red",  
"acp7_c4_m_el":"green",  
...
for index, row in tops.iterrows():
    ...
    plt.suptitle("acp7.rt.vert.zoom"+row["chrom"]+"-"+str(row["start"]))
    plt.savefig("acp7.rt.vert.zoom."+row["chrom"]+"-"+str(row["start"])+".png",
...
for chrom in chromosomes:
    ...
    plt.suptitle("acp7.rt.vert"+chrom)

    plt.savefig("acp7.rt.vert."+chrom+".png",
        dpi=400,transparent=False, bbox_inches='tight', pad_inches = 0)
    plt.close()

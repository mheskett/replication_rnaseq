...
ref_dir=/home/exacloud/gscratch/ThayerLab/heskett/acp_analysis/hg38_fasta_ref
...
bedtools subtract -a $out_dir$filename.plus.cov.bed -b $ref_dir/ucsc.refseq.hg38.txn.whole.gene.plus.chr$chrom.bed \
  > $out_dir$filename.plus.subtract.whole.gene.bed

bedtools subtract -a $out_dir$filename.minus.cov.bed -b $ref_dir/ucsc.refseq.hg38.txn.whole.gene.minus.chr$chrom.bed \
...
bedtools merge -i $out_dir$filename.plus.subtract.whole.gene.bed -d $first_merge | \
  bedtools subtract -a stdin -b $ref_dir/ucsc.refseq.hg38.txn.whole.gene.plus.chr$chrom.bed \
...
bedtools merge -i $out_dir$filename.minus.subtract.whole.gene.bed -d $first_merge | \
  bedtools subtract -a stdin -b $ref_dir/ucsc.refseq.hg38.txn.whole.gene.minus.chr$chrom.bed \
...
bedtools intersect -f 0.25 -v -a $out_dir$filename.tmp1.plus.bed -b $ref_dir/ucsc.refseq.hg38.txn.whole.gene.chr$chrom.bed > $out_dir$filename.tmp2.plus.bed
...
bedtools coverage -a $out_dir$filename.tmp3.plus.bed -b /annotation.files/ucsc.L1s.hg38.chr$chrom.bed -F 0.9 |
...
bedtools intersect -f 0.25 -v -a $out_dir$filename.tmp1.minus.bed -b $ref_dir/ucsc.refseq.hg38.txn.whole.gene.chr$chrom.bed > $out_dir$filename.tmp2.minus.bed
...
bedtools coverage -a $out_dir$filename.tmp3.minus.bed -b $ref_dir/ucsc.L1s.hg38.chr$chrom.bed -F 0.9 |

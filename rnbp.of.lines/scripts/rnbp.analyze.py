...
lines_in_introns = d.intersect(b,f=0.9,s=True) # wb =true to know which intron a line is in
lines_in_vlincs = d.intersect(c,f=0.9,s=True) # wb = true to know which vlinc a line is in

total_intronic_l1s = len(lines_in_introns)
total_vlinc_l1s = len(lines_in_vlincs)

## change this to what fraction of the library itself instead of total number of lines.

#eclip_binding_lines_in_vlincs = a.coverage(lines_in_vlincs,counts=True)


eclip_binding_lines_in_introns = lines_in_introns.intersect(a,wb=True,F=0.9,s=True).to_dataframe(names=["chrom", "start", "stop", "name", 
...
eclip_binding_lines_in_vlincs = lines_in_vlincs.intersect(a,wb=True,F=0.9,s=True).to_dataframe(names=["chrom", "start", "stop", "name", 
																		 "score", "strand","chrom_eclip","start_eclip","stop_eclip","name_eclip","score_eclip",
																		 "strand_eclip","signalvalue","pvalue","qvalue","peak"],
																		 dtype={"chrom":str, "start":int, "stop":int,
																		 "name":str, "score":float, "strand":str,
...
print(eclip_binding_lines_in_vlincs)

print(len(eclip_binding_lines_in_vlincs["name_eclip"].unique()))
print(len(eclip_binding_lines_in_introns["name_eclip"].unique()))

#print(len(eclip_binding_lines_in_introns))
